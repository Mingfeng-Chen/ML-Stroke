...
def cross_validate_data(data, labels, k, params):
    ...
    for i in range(k):
        ...
        if score[0] > best_score:
            best_score = score[0]
            # save
            pickle.dump(model, open("cross_val_best.pkl", "wb"))
...
if __name__ == '__main__':
    ...
    # import pdb;pdb.set_trace()
    # bst.save_model('train_best_%s.model'%(datetime.now()))

    # model = XGBClassifier(**params)
    # model.fit(sampled_train_array, sampled_train_labels)

    # # pred = bst.predict(val_data)
    # pred = model.predict(sampled_train_array)
    # print(pred)
    # # # import pdb;pdb.set_trace()
    # pred = pred > 0.5
    # pred = pred + 0

    # recall = recall_score(pred, sampled_train_labels)
    # print('Recall:', recall)

    # precision = precision_score(pred, sampled_train_labels)
    # print('Precision:', precision)

    # acc = accuracy_score(pred, sampled_train_labels)
    # print('Accuracy:', acc)

    # f1 = f1_score(pred, sampled_train_labels)
    # print('F1:', f1)

    # fpr, tpr, thresholds = roc_curve(pred, sampled_train_labels, pos_label=1)
    # AUC = auc(fpr, tpr)
    # print('AUC:', AUC)
